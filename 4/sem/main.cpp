...
bool checker(const std::string &s, int max_depth, int brackets_num) {
    ...
    return depth == 0 && left_count == brackets_num && right_count == brackets_num;
}
...
int main() {
    std::unordered_map<std::string, std::function<void()>> test_functions_by_name = {
...
void TEST_9() {
    ...
    const std::string WAIT_FOR = "Wait for semaphore.";
...
void TEST_10() {
    ...
    Semaphore forks[num_philosophers] = {1, 1, 1, 1, 1}; // 每个叉子用一个信号量表示
